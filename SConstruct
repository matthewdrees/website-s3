...
def build_post_images(post_in_path, postInfo):
    ...
    for imagename in os.listdir(post_in_path):
        ...
        env.Command((little_image_out, medium_image_out, big_image_out), image_in,
            (f'convert "{image_in}"'
...
def get_images_html(image_files):
    ...
    for image_file in image_files:
        ...
        cp = subprocess.run(['identify', '-format', '%wx%h', image_file], capture_output=True, universal_newlines=True)
        source_image_dimensions = cp.stdout.strip()
        post_image_dimensions_big = post.getPostImageDimensions(source_image_dimensions, 1600, 1200)
        post_image_dimensions_medium = post.getPostImageDimensions(source_image_dimensions, 1024, 768)

        html.append((f'<a href="{postImageUrlNames.big}" data-size="{post_image_dimensions_big}"'
        ...
        html.append(f'<figure>{post.getImageCaptionFromFilename(image_file)}</figure></a>')
